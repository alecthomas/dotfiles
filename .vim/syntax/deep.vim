...
" Keywords
syn keyword pythonStatement	break continue del new
syn keyword pythonStatement	exec return
syn keyword pythonStatement	pass raise
syn keyword pythonStatement	global nonlocal assert
syn keyword pythonStatement	lambda yield
syn keyword pythonStatement	with
syn keyword pythonStatement	function operator constructor destructor class nextgroup=pythonFunction skipwhite
syn match   pythonFunction	"[a-zA-Z_][a-zA-Z0-9_]*" display contained
syn keyword pythonRepeat	for while
syn keyword pythonConditional	if elif else
syn keyword pythonImport	import from as
syn keyword pythonException	try except finally
syn keyword pythonOperator	and in is not or defer deferred ensure super unordered type const
syn keyword pythonTypes		int string nil float self
...
syn region pythonString		start=+"+ skip=+\\\\\|\\"\|\\$+ excludenl end=+"+ end=+$+ keepend contains=pythonEscape,pythonEscapeError,@Spell
syn region pythonString		start=+"""+ end=+"""+ fold keepend contains=pythonEscape,pythonEscapeError,pythonDocTest2,pythonSpaceError,@Spell
syn region pythonString		start=+'''+ end=+'''+ fold keepend contains=pythonEscape,pythonEscapeError,pythonDocTest,pythonSpaceError,@Spell
...
if exists("python_highlight_builtins") && python_highlight_builtins != 0
  ...
  syn keyword pythonBuiltinFunc	execfile file filter frozenset getattr
  ...
  syn keyword pythonBuiltinFunc	slice sorted staticmethod sum tuple
  syn keyword pythonBuiltinFunc	unichr unicode vars xrange zip
endif
